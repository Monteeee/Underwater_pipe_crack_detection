from keras.layers import (Flatten, Dense, Dropout, Input, ZeroPadding2D,
                          Convolution2D, BatchNormalization, Activation,
...
def identity_block(input_tensor, kernel_size, filters, stage, block):
    ...
    x = Convolution2D(
        nb_filter1, 1, 1, name=conv_name_base + '2a', bias=False)(input_tensor)
    ...
    x = Convolution2D(
        nb_filter3, 1, 1, name=conv_name_base + '2c', bias=False)(x)
    x = BatchNormalization(
...
class ResNet50(BaseModel):
    ...
    def _create(self):
       ...
       '''
        ...
        x = Convolution2D(
            64, 7, 7, subsample=(2, 2), name='conv1', bias=False)(x)
        ...
        for i in range(1, 8):
            x = identity_block(
                x, 3, [128, 128, 512], stage=3, block='b' + str(i))

        x = conv_block(x, 3, [256, 256, 1024], stage=4, block='a')
        for i in range(1, 36):
            x = identity_block(
                x, 3, [256, 256, 1024], stage=4, block='b' + str(i))
