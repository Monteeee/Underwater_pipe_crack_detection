...
def return_status(camera):
    ...
    while True:
        ...
        if DEBUG_FLAG:
            ...
            cv.putText(origin, template.format(labels[index], results[index]), (20, 370), cv.FONT_HERSHEY_COMPLEX,
                       0.7,
                       green,
                       2)
        ...
        if frame_nums>9:
            c_list = count_list[len(count_list)-9:-1]
            list_sum = sum(c_list)
            if list_sum >= 4:
                print('pos')
                camera.release()
...
return_status(camera)
# while(True):
#
#     a = return_status(camera)
#     print(a)
